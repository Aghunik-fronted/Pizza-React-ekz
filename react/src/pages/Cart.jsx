  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center text-center py-20 max-w-[600px] mx-auto">
        ...
        <Link to="/" className="bg-[#282828] text-white px-10 py-3 rounded-full font-bold hover:bg-[#444] transition-colors">
            Вернуться назад
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-[820px] mx-auto py-16 px-5">
      <div className="flex justify-between items-center mb-10">
        <div className='flex items-center justify-start gap-[17px]'>
            <svg width="30" height="30" viewBox="0 0 30 30" fill="none">
                <path d="M9.85844 27.9833C11.1931 27.9833 12.2751 26.9014 12.2751 25.5667C12.2751 24.232 11.1931 23.15 9.85844 23.15C8.52375 23.15 7.44177 24.232 7.44177 25.5667C7.44177 26.9014 8.52375 27.9833 9.85844 27.9833Z" stroke="#3F3F3F" strokeWidth="2.8" strokeLinecap="round" strokeLinejoin="round" />
                <path d="M24.3584 27.9833C25.6931 27.9833 26.7751 26.9014 26.7751 25.5667C26.7751 24.232 25.6931 23.15 24.3584 23.15C23.0238 23.15 21.9418 24.232 21.9418 25.5667C21.9418 26.9014 23.0238 27.9833 24.3584 27.9833Z" stroke="#3F3F3F" strokeWidth="2.8" strokeLinecap="round" strokeLinejoin="round" />
                <path d="M7.04294 7.44166H27.9834L25.9534 17.5796C25.8429 18.1359 25.5402 18.6356 25.0984 18.9913C24.6566 19.3469 24.1038 19.5359 23.5367 19.525H10.7646C10.1746 19.53 9.60307 19.319 9.15789 18.9317C8.71272 18.5443 8.42463 18.0076 8.34794 17.4225L6.51127 3.50249C6.43511 2.92157 6.1505 2.3881 5.71039 2.00134C5.27029 1.61459 4.70467 1.40088 4.11877 1.39999H1.40002" stroke="#3F3F3F" strokeWidth="2.8" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
            <h2 className="text-3xl font-bold"> Корзина</h2>   
        </div>
        <button onClick={onClickClear} className="text-[#b6b6b6] font-[16px] cursor-pointer flex gap-[3px]">
      ...
      {items.map((item, index) => (
        <div key={`${item.id}_${item.type}_${item.size}`} className="flex items-center justify-between py-6 border-t border-[#f4f4f4]">
          ...
          <div className="flex items-center gap-[15px]">
              ...
              >-</button>
              <b className="text-[22px]">{item.count}</b>
              <button 
                onClick={() => onPlus(item)}
                className="w-8 h-8 rounded-full border-2 border-[#fe5f1e] bg-transparent text-[#fe5f1e] text-xl font-bold cursor-pointer hover:bg-[#fe5f1e] hover:text-white transition flex items-center justify-center pb-1"
              >+</button>
          ...
          <button onClick={() => onRemove(item)} className="text-gray-300 cursor-pointer">
      ...
      <div className="flex justify-between mt-10">
        <span className='text-[#000000] text-[22px] font-normal'>Всего пицц: <b className='font-bold'>{totalCount} шт.</b></span>
        <span className='text-[22px] text-[#000000] font-normal'>Сумма заказа: <b className="text-[#fe5f1e] font-bold">{totalPrice} ₽</b></span>
      </div>
      <div className="flex justify-between mt-10">
        <Link to="/" className="flex items-center justify-center gap-2 border-2 border-[#D3D3D3] px-8 py-4 rounded-full text-[#cacaca] text-[16px] font-normal hover:bg-[#D3D3D3] hover:text-white transition cursor-pointer">
          <svg width="8" height="12" viewBox="0 0 8 12" fill="none">
            <path d="M7 1L1 6L7 11" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
          Вернуться назад
        </Link>
        <button className="bg-[#FE5F1E] text-white text-[16px] px-10 py-4 rounded-full font-bold hover:bg-[#e24a0e] transition cursor-pointer shadow-lg active:scale-95">
          Оплатить сейчас
        </button>
      </div>
    </div>
